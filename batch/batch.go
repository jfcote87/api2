...
type call struct {
	*googleapi.Call
	// Additional data passed to Response to allow for processing after batch completion.
	tag interface{}
	// if not nil then these credentials are used instead of the Service credentials by
	// setting an Authorization header inside the multipart Part.
...
// Service collects the batch calls via its Queue method and executes them
// via Do().
type Service struct {
...
// createBody process the calls asynchronously and writes the multipart body
// via an io.Pipe.
